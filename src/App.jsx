...
if (currentPercent < 100) { classesToAttend=Infinity; } else { classesToAttend=0; } } else { const denominator=100 -
  requiredPercent; if (denominator <=0) { classesToAttend=Infinity; } else { const x=Math.ceil( (requiredPercent *
  conducted - attended * 100) / denominator ); classesToAttend=Math.max(0, x); } } } let level="neutral" ; if
  ...
  if (requiredPercent === 100 && currentPercent < 100) { level="cooked" ; } return { currentPercent:
    currentPercent.toFixed(2), isSafe, classesToBunk, classesToAttend, level, deficit: conducted - attended // used to
    render explanation }; }; //==========================================// 🚀 MAIN APP COMPONENT
    ...
    if (conducted < 0 || attended < 0 || required < 0) { setError("Negative numbers? Are you time traveling to skip
      ...
      <div className={`min-h-screen font-sans selection:bg-pink-400 selection:text-black transition-colors duration-500
        ...
        <div className="fixed inset-0 pointer-events-none opacity-20" style={{
          backgroundImage: 'radial-gradient(#000 2px, transparent 2px)' , backgroundSize: '30px 30px' }}></div>
                ...
                <div>
                  ...
                  <div className="h-10 w-full border-[4px] border-black bg-gray-200 relative overflow-hidden p-1">
                    ...
                    <div className={`h-full border-2 border-black ${result.isSafe ? 'bg-green-400' : 'bg-red-400' }
                      transition-all duration-1000 ease-out`} style={{ width: `${Math.min(100, Math.max(0,
                      result.currentPercent))}%` }} />

                    <div className="absolute top-0 bottom-0 w-2 bg-black z-10" style={{ left: `${requiredStr}%`,
        ...
        <style dangerouslySetInnerHTML={{__html: ` @keyframes wiggle { 0%, 100% { transform: rotate(-3deg); } 50% {
